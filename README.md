...
| `codex`    | Codex defaults, instructions, and personal skills   | `~/.codex/config.toml`, `~/.codex/AGENTS.md`, `~/.codex/skills/` | all      |
...
- **Codex tracks only portable configuration.** `codex/install.sh` applies the tracked config template while preserving trusted-project paths and hook trust hashes, installs personal agent instructions, and copies tracked personal skills into `~/.codex/skills`. Authentication, history, and sessions remain local under `~/.codex`.
...
- **Edit a config:** edit the file in this repo (or edit the symlinked target — same thing) and commit. For Codex, edit `codex/config.toml`, `codex/AGENTS.md`, or a skill under `codex/skills/`, then rerun `codex/install.sh`.
...
├── codex/                  # Codex config, personal skills, and installer
├── gh/                     # GitHub CLI
